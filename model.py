...
input_img = tf.keras.layers.Input(shape=(120,208,1))
...
x = tf.keras.layers.Dense(128.,kernel_initializer='glorot_uniform')(x)
...
x = tf.keras.layers.Dense(shape[1] * shape[2] * shape[3], kernel_initializer='glorot_uniform',activation='relu')(latent_inputs)
...
x = tf.keras.layers.UpSampling2D(size=(2,2))(x)
x = tf.keras.layers.Conv2D(filters=1,kernel_size=3, activation='sigmoid', padding='same')(x)
...
if __name__ == '__main__':
	...
	reconstruction_loss = tf.keras.losses.binary_crossentropy(K.flatten(input_img), K.flatten(outputs))
	...
	vae.summary()
	checkpoint = tf.keras.callbacks.ModelCheckpoint('vae_cnn.h5', verbose=1,monitor='loss', save_best_only=True, mode='auto',period=1)
	callbacks= [checkpoint]
	batch_size = 100
	limit = len(os.listdir('imgs'))
	spe = int(limit/batch_size)
	print(limit,spe)
	vae.fit_generator(data_generator(batch_size,limit),epochs=100,steps_per_epoch=spe,callbacks=callbacks)
